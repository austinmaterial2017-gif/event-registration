...
公开后端的部署者帐户必须可使用永久注册表，并会拥有它自动建立的活动 Sheet。工作人员与管理员的日常操作通过受签名保护的公开后端执行；需要直接打开某份活动 Sheet 的其他人员，仍须由拥有者另行授予该 Sheet 权限。
...
这次升级只会为**升级后新建**的活动自动建立独立 Sheet，不会扫描或重排旧表。若永久注册表内已经有非空的「活动」「场次」「报名项目」等旧业务资料，`setupSystem()` 会保留原资料，但这些旧活动不会自动拆分、复制或登记到新的活动目录。请先完整备份，再由拥有者制定逐项迁移与核对计划；完成前不要删除旧表或手工伪造活动目录／票券索引。
