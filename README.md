...
- 每个新活动都会自动建立一份独立、私有的 Google Sheet，但只会在管理员最后确认时建立。新活动先保存为只在管理员页面可见的私人草稿；普通“保存草稿”不会建立 Google Sheet。管理员完成活动、场次、座位和报名问题并按下“确认建立活动并生成数据表”后，公开后端才建立及初始化数据表，建立活动的已授权管理员会取得编辑权限。
- 只需在升级后第一次运行时批准一次新增的 Google Drive 权限。
- 以后新增活动不需要建立、修改或重新部署 Apps Script，也不需要输入 Sheet ID；每个活动都使用同一个管理员页面完成。
...
2. 在管理员页建立活动。先按“保存草稿”，再依页面引导配置场次、座位和报名问题；确认无误后按“确认建立活动并生成数据表”。系统此时才自动建立并初始化一份私有 Sheet，并只向已授权管理员显示受保护的 Sheet 链接。生成数据表不会自动开放报名，仍须另行把活动状态改为 `open`。
...
管理员依次建立活动草稿、场次、座位和报名问题，最后确认生成数据表。未生成的草稿可以删除；已生成但完全没有报名、票券或签到记录的活动也可删除，已有历史记录的活动只能结束或归档。参与者页面只显示适合公开的状态：
