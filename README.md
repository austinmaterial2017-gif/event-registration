...
exact schema and matching activity row. `参加者` rows are linked by a unique,
nonblank `participantId` referenced by legacy `报名项目` rows; any other legacy
business tab without an `eventId` relationship is rejected. This check also
runs before a candidate registry can be activated, so advanced switching
cannot bypass migration review.
